...
namespace OmniviewXEngine 
{
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		OMNIVIEW_CORE_ASSERT(size == m_Width * m_Height * bpp, "Data must be entire texture!");
	...
	void OpenGLTexture2D::Bind(uint32_t slot) const
	{
		...
		/*
			...
			glBindTexture(GL_TEXTURE_2D, texture); 一致
			唯一的区别是，第二个版本将改变活动纹理的全局状态。使用2D纹理以外的其他纹理时，
			可能需要调整glBindTexture的第一个参数。
		*/
	}
}
